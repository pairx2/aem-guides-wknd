...
$(document).ready(function () {
    ...
    if (myAccountPage.length > 0 && isOnPublish()) {
        ...
        if (marketingConsectSectionEle.length > 0) {
            ...
            //forget me
            // marketingConsentForgetMeEle = marketingConsectSectionEle.find('#marketingConsent-forget');
...
function initializeUserConsentCheckList(marketingConsectSectionEle, readMode = 1) {
    ...
    if (userConsents && userConsents.length) {
        ...
        const $selectAllCheckbox = formEle.find('.a-checkbox__input[data-select-all=true]'); //
        let formConsents, formConsentsAll;

        /*
         * First condition is executed only if constraints configured in the checkbox dialog (new feature)
         * Else condition is the default execution if dialog is not used (old way with consentsAll)
         */
        if ($selectAllCheckbox.length) {
            const hasGroup = !!$selectAllCheckbox.data('group');
            const groupName = hasGroup ? $selectAllCheckbox.data('group-name') : null;
            formConsents = hasGroup ? formEle.find('.a-checkbox__input[data-group=true][data-group-name='+groupName+']:not([data-select-all=true])') : formEle.find('.a-checkbox__input:not([data-select-all=true])');
            formConsentsAll = formEle.find('.a-checkbox__input[data-select-all=true]');
        }
        else {
            formConsents = formEle.find('.a-checkbox__input:not([name="consentsAll"]):not([name="tncAgree"])');
            formConsentsAll = formEle.find('.a-checkbox__input[name="consentsAll"]');
        }
