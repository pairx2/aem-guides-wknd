...
(function ($, CUI) {
    var GROUP = "colorformat",
        ...
        TITLE = "Color Picker",
        ICON = "textColor",
        PICKER_URL = "/apps/abbott/commons/rte/plugins/popovers/color-picker/_cq_dialog.html";
    ...
    var ColorPickerPlugin = new Class({
        ...
        execute: function (id, value, envOptions) {
            ...
            var tag = CUI.rte.Common.getTagInPath(context, startNode, "span"),
                plugin = this,
                dialog,
                color = jQuery(tag).css("color"),
                dm = ek.getDialogManager(),
                $container = CUI.rte.UIUtils.getUIContainer(jQuery(context.root)),
                propConfig = {
                    'parameters': {
            ...
            function getPickerIFrameUrl(colorValue) {
                var url = PICKER_URL + "?" + REQUESTER + "=" + GROUP;
                
                if (!_.isEmpty(colorValue)) {
                    url = url + "&" + PICKER_NAME_IN_POPOVER + "=" + colorValue;
                }
...
(function ($, $document) {
    ...
    function setupPopoverIframe() {
        var queryParams = queryParameters(),
            $dialog = jQuery("coral-dialog.cq-Dialog");
