(function (ABBOTT) {
    ABBOTT.featuredSlider = (function () {
        ...
        jQuery(document).ready(function() {
            ...
            var $slider = jQuery('.featured-products .featured-products__carousel');
            var products = [];
            var gtmData = [];
            var productSkus = [];
            var dataSku;
            ...
            function setFeaturedProductInfo() {
                products.forEach(function(item) {
                    ...
                    var regularPrice = featuredProduct.siblings('.featured-products__regular-price');
                    var listPrice = featuredProduct.siblings('.featured-products__list-price');
                    var cartButton = featuredProduct.closest('.featured-products__info').find('.featured-products__btn-cart');
                    var pdpLink = featuredProduct.closest('.item').find('figure a').attr('href');

                    if (isOutOfStock) {
                        featuredProduct.closest('.featured-products__info').addClass('obsolete');
                        cartButton.text('OUT OF STOCK').attr('disabled', 'disabled');
                    } else if(item.order_on_call) {
                        cartButton.replaceWith('<a class="btn btn-primary btn-block rounded-0 featured-products__btn-cart" href="' + pdpLink + '">CALL TO ORDER</a>');
                    } else if(item.customizable_options) {
                        cartButton.replaceWith('<a class="btn btn-primary btn-block rounded-0 featured-products__btn-cart" href="' + pdpLink + '">'+ buttonText + '</a>');
                    } else if(isBackorder) { 
                        cartButton.text('BACK ORDER');
                    } else {
                        featuredProduct.closest('.featured-products__info');
            ...
            jQuery(element).owlCarousel({
                loop: (jQuery(element).find('.item').length > 5),
                autoplay: true,
			...
			if(activeLength <= 5 && windowSize >768 ) {
				jQuery(element).parent().find(".featured-products__title").css("margin","0 0 1rem");
            } else if(activeLength <3 && windowSize <768) {
				jQuery(element).parent().find(".featured-products__title").css("margin","0 0 1rem");
			} else{
                jQuery(element).parent().find(".featured-products__title").css("margin","0 3.125rem 1rem");
            }
            ...
            jQuery(element)
                ...
                .on('click.gtm', '.featured-products__btn-cart:enabled', function(e) {   ABBOTT.utils.gtmCartAddHandler(dataSku,gtmData); })
