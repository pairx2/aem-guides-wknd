(function (ABBOTT) {
    ABBOTT.featuredSlider = (function () {
        ...
        jQuery(document).ready(function() {
            ...
            var $slider = jQuery('.featured-products-v2 .featured-products-v2__carousel');
            var products = [];
            var gtmData = [];
            var productSkus = [];
            var dataSku;
             ...
             */
            function pushGTM(productItems) {
                var data = {
                    type: 'impressions',
                    products: productItems.map(function(item) {
                        var $product = $slider.find('.featured-product-sku[value="'+ item.product_sku +'"]').first().closest('.owl-item');
                        var variants = [];
            ...
            function setFeaturedProductInfo() {
                products.forEach(function(item) {
                    ...
                    var regularPrice = featuredProduct.siblings('.featured-products-v2__regular-price');
                    var listPrice = featuredProduct.siblings('.featured-products-v2__list-price');
                    var cartButton = featuredProduct.closest('.featured-products-v2__info').find('.featured-products-v2__btn-cart');
                    var pdpLink = featuredProduct.closest('.item').find('figure a').attr('href');

                    if (isOutOfStock) {
                        featuredProduct.closest('.featured-products-v2__info').addClass('obsolete');
                        cartButton.text('OUT OF STOCK').attr('disabled', 'disabled');
                    } else if(item.order_on_call) {
                        cartButton.replaceWith('<a class="btn btn-primary btn-block rounded-0 featured-products-v2__btn-cart" href="' + pdpLink + '">CALL TO ORDER</a>');
                    } else if(item.customizable_options) {
                        cartButton.replaceWith('<a class="btn btn-primary btn-block rounded-0 featured-products-v2__btn-cart" href="' + pdpLink + '">'+ buttonText + '</a>');
                    } else if(isBackorder) { 
                        cartButton.text('BACK ORDER');
                    } else {
                        featuredProduct.closest('.featured-products-v2__info');
			...
			if(activeLength <= 4 && windowSize >768 ) {
				jQuery(element).parent().find(".featured-products-v2__title").css("margin","0 0 1rem");
            } else if(activeLength <3 && windowSize <768) {
				jQuery(element).parent().find(".featured-products-v2__title").css("margin","0 0 1rem");
			} else{
                jQuery(element).parent().find(".featured-products-v2__title").css("margin","0 3.125rem 1rem");
            }
            // Add event Listener to elements
            jQuery(element)
                .on('click.cartadd', '.featured-products-v2__btn-cart', function(e) {
                    var data = {
                ...
                .on('click.gtm', '.featured-products-v2__btn-cart:enabled', function(e) {   ABBOTT.utils.gtmCartAddHandler(dataSku,gtmData); })
            ...
            function applyCarouselDesign(){
                var total = $('.featured-products-v2 .featured-products-v2__carousel .owl-stage .owl-item.active').length;
