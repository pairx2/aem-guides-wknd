...
function UpdateSignUpRequest(formData){
    ...
    terms = $('input[value="ch3"]').is(':checked')? 'true' : 'false';
    if (windowWidth >= 1200) {
        source = 'DESKTOP';
    }
    ...
    let question1 = $('[name="question1"]').closest('.a-dropdown').find('.a-dropdown__title-text').text()?.trim();
    let question2 = $('[name="question2"]').closest('.a-dropdown').find('.a-dropdown__title-text').text()?.trim();
    let question1Value = $('[name="question1"]').find('.selected span').text()?.trim();
    let question2Value = $('[name="question2"]').find('.selected span').text()?.trim();
    let answerId1 = $('[name="question1"]').find('.selected')?.attr('data-optionvalue');
    let answerId2 = $('[name="question2"]').find('.selected')?.attr('data-optionvalue');
    ...
    formData.body = {
            ...
            "dob": `${formData.body.year}-${getMonthFromString(formData.body.month)}-01`,
            "additionalProperties": {
                "clientAnalyticsId": clientId,
                "consentReceiveInformationGlucerna": check1
            }
        },        
        "surveyQuestions": [
            {
...
function signUpError(responseData){
    if(responseData.errorCode != 0) {
        if(responseData.response.i18nMessageKey == "REG-USER-1030"){
            $("#signup-form .o-wizard-container__error-msg").text($('[name="alreadyexist"]').val());
        }else{
            $("#signup-form .o-wizard-container__error-msg").text(responseData.response?.statusReason);  
        }
    }
}
