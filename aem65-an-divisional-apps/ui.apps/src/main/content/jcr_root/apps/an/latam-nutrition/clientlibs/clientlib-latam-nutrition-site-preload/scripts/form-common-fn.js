...
function radioValue(element) {
  ...
  newArr1 = $.grep(element, function (n, i) {
    let isConsentValueTrue = Boolean(n.consentValue);
    return isConsentValueTrue;
  });
...
function processResponse(response) {
  let isStatusFalse = response.status === false;
  if (isStatusFalse ) {
    return false;
...
function getCountryLang() {
  ...
  let result =langRegex.exec(currPageUrl);
  if (result != null) {
    return result[1];
  }
  return '';
...
function getCookie(cname, isLang = false) {
  ...
  for (let c of ca) {
    ...
    if (cc.startsWith(name)) {
      return cc.substring(name.length, c.length);
    }
  }
  return '';
}
