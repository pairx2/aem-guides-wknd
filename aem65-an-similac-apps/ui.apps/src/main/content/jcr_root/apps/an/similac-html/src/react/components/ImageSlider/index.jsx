...
import { onSwiped, LEFT, RIGHT } from 'react-swipeable';
...
class ImageGallery extends React.Component {
  ...
  constructor(props) {
    ...
    this.thumpPreload = this.thumpPreload.bind(this);
    this.resize = debounce(this.resize.bind(this), 350);
  ...
  resize() {
    this.setState({ winWidth: getWidth() }, () => {
  ...
  onSliding() {
    ...
    let videoFrame = jQuery(".image-gallery-slide.center").find(".product-carosal-video");
    if(videoFrame.length > 0){
      let idv = videoFrame.attr("id");
   ...
   calculatePercValue = (percTime)=> {
    ...
    getYoutubePlayPercentage = (target)  => {
      var PercentageValue = "";
      try {
        ...
        PercentageValue = this.calculatePercValue(percTime);
      } catch (err) {}
      return PercentageValue;
    }
    ...
    onPlayerStateChange = (event) => {
      ...
      var videoPercent;
      var videoTitle = jQuery(".image-gallery-slide.center").find(".product-carosal-video").attr("data-event-label");
      videoTitle = ABBOTT.utils.hyphenWords(videoTitle);
      switch (playerStatus) {
        case 0: {
          // Ended
          videoPercent = this.getYoutubePlayPercentage(event.target);
  ...
  onThumbnailClick(event, index) { 
    ABBOTT.gtm.buildAndPush.formTracking('products-image-slider', 'select', jQuery("#pdp-info").data("product-name"));
  ...
  getAlignmentClassName(index) {
    // Function for lazing loading
    const { currentIndex } = this.state;
    const { infinite, items, autoPreload = false } = this.props;
    let alignment = '';
    ...
    switch (index) {
      case (currentIndex - 1):
        alignment = autoPreload ? ` ${leftClassName}` : '';
        break;
      case (currentIndex):
        alignment = ` ${centerClassName}`;
        break;
      case (currentIndex + 1):
        alignment = autoPreload ? ` ${rightClassName}` : '';
        break;
      default:
        break;
    }

    if (items.length >= 3 && infinite && autoPreload) {
  ...
  thumpPreload(currentIndex, thumbnailPreloadCount, totalSlides) {
    var subarray = [];
    for (var i = 0; i < thumbnailPreloadCount; i++) {
      var ind = (currentIndex + i /*+ shiftIndex*/) % totalSlides;
      if (ind < 0) {
        subarray.push(totalSlides + ind);
      }
      else {
        subarray.push(ind);
      }
  ...
  getSlideItems() {
    ...
    const thumbnails = [];
    const bullets = [];
    const thumpPreloadInfo = this.thumpPreload(currentIndex, thumpPreloadCount, items.length);
    ...
    items.forEach((item, index) => {
      ...
      if (infinite) {
        // FYI : Don't add some slides while transitioning to avoid background transitions
      ...
      if (showThumbnails && this.state.winWidth > this.props.showThumbnailsAboveWidth) {
        ...
        const showThumpItem = !lazyLoad || thumpPreloadInfo.includes(index) || this.lazyLoadedThump[index];
        if (showThumpItem && lazyLoad && !this.lazyLoadedThump[index]) {
          this.lazyLoadedThump[index] = true;
        }
        ...
        thumbnails.push(
          ...
          >
            {showThumpItem ? handleRenderThumbInner(item) : handleRenderItem({
              thumbnail: this.props.thumbPreloadImage,
  ...
  slideToIndex(index, event) {
    ...
    if (!isTransitioning) {
      ...
      }, () => this.onSliding() && this.handleResize());
    }
  }
    ...
    if (items.length === 2) {
      ...
      */
      this.setState({
        ...
        // Added 25ms timeout to avoid delay in moving slides over
    ...
    return (
      <div>
        {item.type == 'image' ? (
          ...
          </div>) : (<div> <iframe  id={item["video_id"]} data-event-label={item["video_title"]}  src={item["video_content"]}   className="product-carosal-video" allowFullScreen></iframe></div>)}
        {
          item.description && (
