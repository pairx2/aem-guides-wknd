...
class Registration extends React.Component {
  constructor(props) {
    ...
    props.data.fields.map((data)=>{
      ...
      if(data.name === 'captchaAction'){
        this.captchaActionObj = data.value;
      }
    });
        ...
        <Formik
          ...
          onSubmit={ async (values, actions) => {
            values.captchaType = reCaptchaVType;
             values.captchaValue = await getCaptchaToken(this.props.data.enableRecaptcha, this.gSiteKey, "fast-registration");
            if (values.emailAddress) {
              values.captchaAccountId = window.btoa(values.emailAddress);
              values.captchaAction = this.captchaActionObj;
