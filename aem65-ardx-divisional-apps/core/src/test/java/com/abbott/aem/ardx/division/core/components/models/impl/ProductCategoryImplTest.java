...
import com.abbott.aem.ardx.division.core.components.models.ProductCategory;
...
class ProductCategoryImplTest {
	...
	void setUp() throws Exception {
		apiLookupService = Mockito.mock(APILookupService.class);
		 when(apiLookupService.getAPIEndpointForKey("siteSearch")).thenReturn("eslendpoint");
	...
	@Test
	void testGetESLEndpoint() {
		ctx.currentResource(ProductCategoryImplTest.PATH);
		ProductCategory productCategory = ctx.request().adaptTo(ProductCategory.class);
		assertEquals("eslendpoint", productCategory.getEslEndPoint());
	}
