$(document).ready(function () {
    ...
    $(_headerClass[0]).find(".xf-content-height").addClass("escreen-header");
    ...
    const sidveNaveActive =$("#columncontainer").find(".navigation").find(".nav-link");
    activeLink(sidveNaveActive)
    const sideNative_title =$("#columncontainer").find(".columncontrol__column");
    const sideNavActive_title = $(sideNative_title[0]).find(".cmp-title__link");
    activeLink(sideNavActive_title)
    ...
    $(navigationLink).hover(function(){
        ...
        {
            activeLink($(subNavigation).find("a"));
