$(function () {
    ...
    $(submitSearch).click(function (e) {
        e.preventDefault();
        const parent_country = $('.lang-search-country fieldset.drop-down').find('.a-dropdown__container').find('select');
        const parent_language = $('.lang-search-language fieldset.drop-down').find('.a-dropdown__container').find('select');
        ...
        if (parent_country.find('option:selected').val() !== curr_cntry ||
            ...
            if (isFormValid) {
                ...
                if (window.location.href.indexOf("manuals") != -1) {
                    $search.setLocalStorage('curr-country-cd', parent_country.find('option:selected').val());
                    $search.setLocalStorage('curr-country', parent_country.find('option:selected').val());
                    $search.setLocalStorage('curr-country-manuals', parent_country.find('option:selected').text());
                    $search.setLocalStorage('curr-lang', parent_language.find('option:selected').val());
                    let langmanuals = parent_language.find('option:selected').val();
                    let langcode = $formsManuals.getLanguageCodeManuals(langmanuals);
                    $search.setLocalStorage('curr-lang', langcode);
                    $search.setLocalStorage('selectedCountry', parent_country.find('option:selected').attr('data-country-cd'));
...
function sortDropdown(ulclass, liclass) {
    var eleUl = $(ulclass).find(liclass);
