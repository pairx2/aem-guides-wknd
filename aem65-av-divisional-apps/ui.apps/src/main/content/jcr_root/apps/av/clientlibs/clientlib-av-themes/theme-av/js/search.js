                ...
                if (r.country == $search.getLocalStorage('selectedCountry')
                    ...
                    if (i < limit) {
                        ...
                        if (window.location.href.indexOf("manuals") != -1 && $('#externalLink').length) {
                            $(recentlyViewedTile).find('.a-tile__title-text').html('<p>' + r.productname + '</p>');
                        }
    ...
    const setSearchResultsContainer = function (data, msg, start, end, searchTokenTemplate, isFilter) {
        ...
        if (data && data.length) {
            ...
            if ($search.checkIfSearchPage() && isFilter) {
                ...
                let filteredData = [];
                data.map(ct => {
                    const prodName = ct.searchFilterProdName.split(',');
                    if (prodName && prodName.length) {
                        prodName.map(tag => {
                            if (role == 'HCP') {
                ...
                if (filteredData.length) {
                    let dupVersionids = filteredData.map(e => e.versionid)
                        .map((e, i, final) => final.indexOf(e) !== i && i)
                        .filter(obj => filteredData[obj])
                        .map(e => filteredData[e].versionid)
                        .filter((value, index, self) => {
                            return self.indexOf(value) === index;
                        });
                    if (dupVersionids.length) {
                        dupVersionids.forEach((id) => {
                            let dupProdsArr = filteredData.filter((prod) => prod.versionid == id);
...
$(document).on('click', '#previousPage', async function (e) {
    ...
    const favoriteProductCategories = $search.getLocalStorage('favoriteProductCategories');
    let searchResults = $search.getLocalStorage('searchResults');
    ...
    if (prev_st && prev_end) {
        ...
        if (window.location.href.indexOf("manuals") != -1) {
            if ($searchManuals.checkIfSearchPage()) {
                $search.initProductsContainer($globals.noSearchResultText, productCategories, filteredResults, prev_st, prev_end);
            }
        }
        if ($search.checkIfSearchPage()) {
            $search.initProductsContainer($globals.noSearchResultText, productCategories, filteredResults, prev_st, prev_end);
...
$(document).on('click', '#nextPage', async function (e) {
    ...
    if (next_st && next_end) {
        ...
        if (window.location.href.indexOf("manuals") != -1) {
            if ($searchManuals.checkIfSearchPage()) {
                $search.initProductsContainer($globals.noSearchResultText, productCategories, filteredResults, next_st, next_end);
            }
        }
