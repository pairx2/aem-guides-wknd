...
public final class InfoWriter {
    ...
    public void message(String message, final Object... vars) {
        if (ArrayUtils.isEmpty(vars)) {
            ...
            for (final Object placeholder : vars) {
                try {
                    message = StringUtils.replaceOnce(message, "{}", TypeUtil.toString(placeholder));
				} catch (InvocationTargetException | IllegalAccessException | NoSuchMethodException e) {
					log.error("Could not derive a valid String representation for {} using TypeUtil.toString(..)", placeholder,
							e);
