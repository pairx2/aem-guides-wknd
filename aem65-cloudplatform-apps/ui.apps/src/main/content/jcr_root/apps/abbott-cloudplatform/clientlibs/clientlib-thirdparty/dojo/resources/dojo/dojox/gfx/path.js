define("dojox/gfx/path", ["./_base", "dojo/_base/lang","dojo/_base/declare", "./matrix", "./shape"],
	...
	var Path = declare("dojox.gfx.path.Path", shapeLib.Shape, {
		...
		_getRealBBox: function(){
			...
			var bbox = this.bbox, matrix_text = this._getRealMatrix();
			this.bbox = null;
			for(var i = 0, len = this.segments.length; i < len; ++i){
				this._updateWithSegment(this.segments[i], matrix_text);
			}
			var t = this.bbox;
			this.bbox = bbox;
			this.tbbox = t ? [
		...
		_updateWithSegment: function(segment, matrix_text_c){
			// summary:
			...
			switch(segment.action){
				...
				case "T":
					for(i = 0; i < l; i += 2){
						this._updateBBox(n[i], n[i + 1], matrix_text_c);
					}
					this.last.x = n[l - 2];
					this.last.y = n[l - 1];
					this.absolute = true;
				...
				case "H":
					for(i = 0; i < l; ++i){
						this._updateBBox(n[i], this.last.y, matrix_text_c);
					}
					this.last.x = n[l - 1];
					this.absolute = true;
					break;
				case "V":
					for(i = 0; i < l; ++i){
						this._updateBBox(this.last.x, n[i], matrix_text_c);
					}
					this.last.y = n[l - 1];
					this.absolute = true;
					break;
				case "m":
					var start = 0;
					if(!("x" in this.last)){
						this._updateBBox(this.last.x = n[0], this.last.y = n[1], matrix_text_c);
						start = 2;
					}
					for(i = start; i < l; i += 2){
						this._updateBBox(this.last.x += n[i], this.last.y += n[i + 1], matrix_text_c);
					}
					this.absolute = false;
					break;
				case "l":
				...
				case "h":
					for(i = 0; i < l; ++i){
						this._updateBBox(this.last.x += n[i], this.last.y, matrix_text_c);
					}
					this.absolute = false;
					break;
				case "v":
					for(i = 0; i < l; ++i){
						this._updateBBox(this.last.x, this.last.y += n[i], matrix_text_c);
					}
					this.absolute = false;
					break;
				case "c":
					for(i = 0; i < l; i += 6){
						this._updateBBox(this.last.x + n[i], this.last.y + n[i + 1], matrix_text_c);
						this._updateBBox(this.last.x + n[i + 2], this.last.y + n[i + 3], matrix_text_c);
						this._updateBBox(this.last.x += n[i + 4], this.last.y += n[i + 5], matrix_text_c);
					}
					this.absolute = false;
					break;
				case "s":
				...
				case "A":
					for(i = 0; i < l; i += 7){
						this._updateBBox(n[i + 5], n[i + 6], matrix_text_c);
					}
					this.last.x = n[l - 2];
					this.last.y = n[l - 1];
					this.absolute = true;
				...
				case "a":
					for(i = 0; i < l; i += 7){
						this._updateBBox(this.last.x += n[i + 5], this.last.y += n[i + 6], matrix_text_c);
					}
					this.absolute = false;
					break;
			}
