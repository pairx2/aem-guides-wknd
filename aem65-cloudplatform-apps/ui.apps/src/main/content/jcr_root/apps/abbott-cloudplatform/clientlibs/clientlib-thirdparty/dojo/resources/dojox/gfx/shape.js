define("dojox/gfx/shape", ["./_base", "dojo/_base/lang", "dojo/_base/declare", "dojo/_base/kernel", "dojo/_base/sniff",
	...
	shape.Shape = declare("dojox.gfx.shape.Shape", null, {
		...
		setShape: function(shape_text){
			...
			// COULD BE RE-IMPLEMENTED BY THE RENDERER!
			this.shape_text = g.makeParameters(this.shape_text, shape_text);
			this.bbox = null;
			return this;	// self
		},
	...
	shape.Container = {
		...
		add: function(shape_add){
			// summary:
			//		adds a shape to the list
			// shape: dojox/gfx/shape.Shape
			//		the shape to add to the list
			var oldParent = shape_add.getParent();
			if(oldParent){
				oldParent.remove(shape_add, true);
			}
			this.children.push(shape_add);
			return shape_add._setParent(this, this._getRealMatrix());	// self
		},
		remove: function(shape_remove, silently){
			// summary:
			...
			for(var i = 0; i < this.children.length; ++i){
				if(this.children[i] == shape_remove){
					if(silently){
						// skip for now
					}else{
						shape_remove.parent = null;
						shape_remove.parentMatrix = null;
					}
		...
		clear: function(/*Boolean?*/ destroy){
			...
			var shape_matrix;
			for(var i = 0; i < this.children.length;++i){
				shape_matrix = this.children[i];
				shape_matrix.parent = null;
				shape_matrix.parentMatrix = null;
				if(destroy){
					shape_matrix.destroy();
				}
			}
			this.children = [];
			return this;	// self
		...
		getBoundingBox: function(){
			...
			if(this.children){
				...
				var result = null;
				arr.forEach(this.children, function(shape_lib){
					var bb = shape_lib.getBoundingBox();
					if(bb){
						var ct = shape_lib.getTransform();
						if(ct){
		...
		// moving child nodes
		_moveChildToFront: function(shape_front){
			// summary:
			//		moves a shape to front of the list of shapes
			// shape: dojox/gfx/shape.Shape
			//		one of the child shapes to move to the front
			for(var i = 0; i < this.children.length; ++i){
				if(this.children[i] == shape_front){
					this.children.splice(i, 1);
					this.children.push(shape_front);
					break;
				}
			}
			return this;	// self
		},
		_moveChildToBack: function(shape_back){
			// summary:
			//		moves a shape to back of the list of shapes
			// shape: dojox/gfx/shape.Shape
			//		one of the child shapes to move to the front
			for(var i = 0; i < this.children.length; ++i){
				if(this.children[i] == shape_back){
					this.children.splice(i, 1);
					this.children.unshift(shape_back);
					break;
				}
			}
			return this;	// self
	...
	shape.Ellipse = declare("dojox.gfx.shape.Ellipse", shape.Shape, {
		...
		getBoundingBox: function(){
			...
			if(!this.bbox){
				var shape_box = this.shape;
				this.bbox = {x: shape_box.cx - shape_box.rx, y: shape_box.cy - shape_box.ry,
					width: 2 * shape_box.rx, height: 2 * shape_box.ry};
			}
			return this.bbox;	// dojox/gfx.Rectangle
		}
	});
	...
	shape.Circle = declare("dojox.gfx.shape.Circle", shape.Shape, {
		...
		getBoundingBox: function(){
			...
			//		returns the bounding box
			if(!this.bbox){
				var shape_circle = this.shape;
				this.bbox = {x: shape_circle.cx - shape_circle.r, y: shape_circle.cy - shape_circle.r,
					width: 2 * shape_circle.r, height: 2 * shape_circle.r};
			}
			return this.bbox;	// dojox/gfx.Rectangle
		}
	});
	...
	shape.Line = declare("dojox.gfx.shape.Line", shape.Shape, {
		...
		getBoundingBox: function(){
			...
			//		returns the bounding box
			if(!this.bbox){
				var shape_bound = this.shape;
				this.bbox = {
					x:		Math.min(shape.x1, shape_bound.x2),
					y:		Math.min(shape.y1, shape_bound.y2),
					width:	Math.abs(shape.x2 - shape_bound.x1),
					height:	Math.abs(shape.y2 - shape_bound.y1)
				};
			}
			return this.bbox;	// dojox/gfx.Rectangle
		}
	...
	shape.Creator = {
		// summary:
		//		shape creators
		createShape: function(shape_create){
			// summary:
			...
			switch(shape_create.type){
				case g.defaultPath.type:		return this.createPath(shape_create);
				case g.defaultRect.type:		return this.createRect(shape_create);
				case g.defaultCircle.type:	    return this.createCircle(shape_create);
				case g.defaultEllipse.type:	    return this.createEllipse(shape_create);
				case g.defaultLine.type:		return this.createLine(shape_create);
				case g.defaultPolyline.type:	return this.createPolyline(shape_create);
				case g.defaultImage.type:		return this.createImage(shape_create);
				case g.defaultText.type:		return this.createText(shape_create);
				case g.defaultTextPath.type:	return this.createTextPath(shape_create);
			}
			return null;
		},
