...
class ProductsList {
  ...
  init() {
    ...
    const { products, ...messages } = productsData;
  ...
  attachEvents() {
    ...
    this._container.querySelector('.productlist-show-all').addEventListener('click', () => {
      this.showAll();
    });
  ...
  injectFixedTranslations() {
    ...
    this._container.querySelector('.productlist-show-all').innerHTML = this._messages['showAllResults'];
  ...
  addPage(resetContent = false) {
    ...
    this._filterList
      ...
      .forEach(({ detailLink, image, title, description }) => {
        newListItems += `
          <li class="productlist-item">
            <a class="productlist-page" href="${detailLink}">
