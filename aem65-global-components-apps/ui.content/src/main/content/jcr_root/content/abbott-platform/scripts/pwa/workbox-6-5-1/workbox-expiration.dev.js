var workbox = workbox || {};
workbox.expiration = (function (exports, assert_js, dontWaitFor_js, logger_js, WorkboxError_js, cacheNames_js, getFriendlyURL_js, registerQuotaErrorCallback_js) {
  'use strict';
  ...
  function getMethod(target, prop) {
    ...
    const method = async function (storeName, ...args) {
      ...
      let targetMethod = tx.store;
      if (useIndex) targetMethod = targetMethod.index(args.shift()); // Must reject if op rejects.
      // If it's a write operation, must reject if tx.done rejects.
      // Must reject with op rejection first.
      // Must resolve with op value.
      // Must handle both promises (no unhandled rejections)

      return (await Promise.all([targetMethod[targetFuncName](...args), isWrite && tx.done]))[0];
    };
