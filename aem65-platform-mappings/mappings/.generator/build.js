...
const generateSites = function () {
  ...
  for (let i = 0; i < resources.length; i++) {
    ...
    //Uncomment to log list of sites.
    /**
    fs.writeFile('list-of-sites.log', "", err => {
        if (err) {
        console.error(err);
        }
    });
    for (let j=0; j< entry.markets.length; j++){
        for(let k=0; k< entry.markets[j].languages.length; k++){
            console.log(entry.division+"::"+entry.name+"/"+entry.markets[j].path+"/"+entry.markets[j].languages[k]);
            fs.appendFile('list-of-sites.log', entry.division+"::"+entry.name+"/"+entry.markets[j].path+"/"+entry.markets[j].languages[k]+"\n", err => {
                if (err) {
                console.error(err);
                }
            });
  ...
  entry.isCloudSubdomainRequiredOnUat = parseBoolean(entry.isCloudSubdomainRequiredOnUat)
    ...
    env.mapDirName.forEach( envName => {
      ...
      if (envName == 'prod') {
        ...
        if (entry.isCloudSubdomainRequiredOnUat) {
